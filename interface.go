...
var (
    ...
    Sentinela = Elemento{'Σ',CorVerde,CorPadrao,true}
)
...
func interfaceDesenharBarraDeStatus(status string, score int) {
    for i, c := range status {
        termbox.SetCell(i, 0, c, CorTexto, CorPadrao)
    }

    scoreMsg := fmt.Sprintf("Moedas: %d", score)
...
func interfaceDesenharJogo(jogo *Jogo) {
    //garante leitura consistente de todos os dados
    jogo.Mu.RLock()
    defer jogo.Mu.RUnlock()

    interfaceLimparTela()
    interfaceDesenharBarraDeStatus(jogo.StatusMsg, jogo.Score)
